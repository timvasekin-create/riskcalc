...
- Ордера в алертах (сайт + бот): «ORDER REMOVED — #86» с направлением
  ...
  ВАЖНО: направление берётся от ПОЗИЦИИ (dir_label), а не от стороны ордера —
  у лонга TP/SL продаются (HL: сторона A), но для юзера это Long (иначе
  получалась метка SHORT на тейке лонга — баг от 26.09). Если позиции уже нет,
  продажа трактуется как закрытие лонга. Входные лимитки — по стороне ордера.
  Окно склейки снятых ордеров: бот собирает их ORDER_GROUP_WINDOW = 25 с
  ...
  dir_label. У бота и сайта ордера сравниваются по oid — частичное исполнение
  больше не даёт ложных «сняли + поставили заново». Тост на сайте многострочный
  (лента — одной строкой).
